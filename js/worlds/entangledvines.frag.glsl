// Since no specific license was applied to shader https://www.shadertoy.com/view/MlBSDW
// it falls under the site default license:
// This work is licensed under a Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License.
// Created by Shane https://www.shadertoy.com/user/Shane
...
uniform float magicNumberX;
uniform float magicNumberY;
uniform float magicNumberZ;
...
vec3 firePalette(float i){
    ...
    vec3 L = vec3(4.4, 5.6, 7.4); // Red, green, blue wavelengths (in hundreds of nanometers).
...
float map(vec3 p) {
    ...
    vec2 perturb = vec2(sin((p.z * 2.15 + p.x * 2.35)), cos((p.z * 1.15 + p.x * 1.25))) * magicNumberX;
    vec2 perturb2 = vec2(cos((p.z * 1.65 + p.y * 1.75)), sin((p.z * 1.7 + p.y * 1.6))) * magicNumberX;
	vec2 q1 = mod(p.xy + vec2(0.25, -0.5), magicNumberY) - 1.0 + perturb*vec2(0.25, 0.5);
	vec2 q2 = mod(p.yz + vec2(0.25, 0.25), magicNumberY) - 1.0 - perturb*vec2(0.25, 0.3);
	vec2 q3 = mod(p.xz + vec2(-0.25, -0.5), magicNumberY) - 1.0 - perturb2*vec2(0.25, 0.4);
	
	// Used to add some bumps on the overall structure. See "p.x*p.y*p.z.. etc" below.
	p = sin(p*8. + cos(p.yzx*8.)) * magicNumberX;
	
	// The cylinders.
	float s1 = length( q1 ) - 0.24*magicNumberZ; // max(abs(q1.x), abs(q1.y)) - 0.2; // etc.
	float s2 = length( q2 ) - 0.24*magicNumberZ;
	float s3 = length( q3 ) - 0.24*magicNumberZ;
...
float trig3(in vec3 p){
    p = cos(p*2. + (cos(p.yzx) + 1. + slowTime*4.)*1.57);
...
void main() {    
    ...
    vec3 lp = vec3(0., 0., -FAR + lightOffset); //+ ro + rd*10.;
